...
export class HistoriquePage implements OnInit{
  ...
  today = new Date(); //Variable pour savoir si la date du trajet est passée
              ...
              tra.forEach(value3=> {
                if(value3['tra_id']==value2['uti_tra_idTra']){
                  ...
                  if(newDate<this.today){
                    ...
                    if(that.uti_tra2 != null){
                      that.uti_tra2.subscribe(uti_tras2 =>{
                        uti_tras2.forEach(uti_traBIS2=> {
                          
                          if(value3['tra_id']==uti_traBIS2['uti_tra_idTra']){
                            if(uti_traBIS2['uti_tra_idUti'] != this.userid){
                              //console.log(uti_traBIS2)
                              this.liste_users.push({trajet:value3, user:uti_traBIS2['uti_tra_idUti'], role:uti_traBIS2['uti_tra_role']});
                            }
                          }
                        })
                      })
